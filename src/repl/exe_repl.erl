...
-export([local_allowed/3, non_local_allowed/3,q/2,banner/0]).
...
format_strs(TextAttrs) ->
	F = fun(AttrTuple) ->
			...
			FmtStr = etcol:t([ { Attrs, "~ts"}, { [{text_attr,reset}], ""}]),
...
local_allowed(q,[],State) -> {true,State};
local_allowed(_,_,State)  -> {false,State}.

non_local_allowed({init,stop},[],State) -> {true,State};
non_local_allowed(_,_,State)            -> {false,State}.
...
whereis_evaluator() ->
    case whereis(user) of undefined -> undefined;
	                           User -> case group:interfaces(User) of
                                            [] -> case user:interfaces(User) of
                                                       [] -> undefined;
                                                       [{shell,Shell}] -> whereis_evaluator(Shell) end;
               [{user_drv,UserDrv}] -> case user_drv:interfaces(UserDrv) of
                                            [] -> undefined;
                       [{current_group,Group}] -> GrIfs = group:interfaces(Group),
                                                  case lists:keyfind(shell, 1, GrIfs) of
                                                       {shell, Shell} -> whereis_evaluator(Shell);
                                                                false -> undefined end end end end.
...
stop_restricted() -> application:unset_env(stdlib, restricted_shell), exit(restricted_shell_stopped).
...
banner() -> "\e[38;2;187;187;187mGroupoid Infinity \e[38;2;13;13;136mEXE\e[38;2;187;187;187m Programming Language \e[38;2;13;13;136mv1.0\e[0m".
...
report_exception(Class, Severity, {Reason,Stacktrace}, RT) ->
    ...
    io:requests([{put_chars, Tag},
                 {put_chars,
                  format_exception(I, Class, Reason, Stacktrace, SF, PF)},
                 nl]).
...
format_errstr_call(ErrStr, Class, ForMForFun, As, PF, Pre0) ->
    Pre1 = [Pre0 | lists:duplicate(exited_size(Class),$\s)],
...
count_nl([E | Es])                -> count_nl(E) + count_nl(Es);
count_nl($\n)                     -> 1;
count_nl(Bin) when is_binary(Bin) -> count_nl(binary_to_list(Bin));
count_nl(_)                       -> 0.

argss(0) -> <<"no arguments">>;
argss(1) -> <<"one argument">>;
argss(2) -> <<"two arguments">>;
argss(I) -> io_lib:fwrite(<<"~w arguments">>, [I]).
...
mfa_to_string(M, F, A) -> io_lib:fwrite(<<"~s/~w">>, [mf_to_string({M, F}, A), A]).
mf_to_string({M, F}, A) ->
    case erl_internal:bif(M, F, A) of
        true -> io_lib:fwrite(<<"~w">>, [F]);
        false ->
            case is_op({M, F}, A) of
                {yes, '/'} -> io_lib:fwrite(<<"~w">>, [F]);
                {yes, F} -> atom_to_list(F);
                no -> io_lib:fwrite(<<"~w:~w">>, [M, F]) end end;
mf_to_string(Fun, _A) when is_function(Fun) -> format_fun(Fun);
mf_to_string(F, _A)                         -> io_lib:fwrite(<<"~w">>, [F]).

n_args(A) when is_integer(A) -> A;
n_args(As) when is_list(As)  -> length(As).

analyze_exception(error, Term, Stack) ->
    case {is_stacktrace(Stack), Stack, Term} of
         {true, [{_,_,As,_}=MFAL|MFAs], function_clause} when is_list(As) -> {Term,[MFAL],MFAs};
         {true, [{shell,F,A,_}], function_clause} when is_integer(A) -> {Term, [{F,A}], []};
         {true, [{_,_,_,_}=MFAL|MFAs], undef} -> {Term,[MFAL],MFAs};
         {true, _, _} -> {Term,[],Stack};
         {false, _, _} -> {{Term,Stack},[],[]} end;
analyze_exception(_Class, Term, Stack) ->
    case is_stacktrace(Stack) of
         true -> {Term,[],Stack};
        false -> {{Term,Stack},[],[]} end.
...
format_exception(I, Class, Reason, StackTrace, StackFun, FormatFun) 
    ...
    S = lists:duplicate(I-1,$\s),
    {Term,Trace1,_Trace} = analyze_exception(Class, Reason, StackTrace),
    Expl0 = explain_reason(Term, Class, Trace1, FormatFun, S),
    io_lib:fwrite(<<"~s~s">>, [exited(Class), Expl0]).
...
local_func(f, [_Other], _Bs, _Shell, _RT, _Lf, _Ef) ->
    erlang:raise(error, function_clause, [{exe_repl,f,1}]);
...
local_func(rd, [_,_], _Bs, _Shell, _RT, _Lf, _Ef) ->
    erlang:raise(error, function_clause, [{exe_repl,rd,2}]);
...
local_func(rp, [A], Bs0, _Shell, RT, Lf, Ef) ->
    ...
    io:requests([{put_chars,
                  exe_pretty:print(V, 1, W, -1, ?CHAR_MAX,
                                      record_print_fun(RT))},
                 nl]),
    {value,ok,Bs};
...
local_func(which, [_Other], _Bs, _Shell, _RT, _Lf, _Ef) ->
    erlang:raise(error, function_clause, [{exe_repl,which,1}]);
local_func(import, [M], Bs, _Shell, _RT, _Lf, _Ef) ->
    case erl_parse:package_segments(M) of
	error -> erlang:raise(error, function_clause, [{exe_repl,import,1}]);
	M1 ->
		...
		false ->
		    exit({{bad_module_name, Mod}, [{exe_repl,import,1}]})
	    end
    end;
local_func(import_all, [P], Bs0, _Shell, _RT, _Lf, _Ef) ->
    case erl_parse:package_segments(P) of
	error -> erlang:raise(error, function_clause, [{exe_repl,import_all,1}]);
	P1 ->
...
local_func(history, [_Other], _Bs, _Shell, _RT, _Lf, _Ef) ->
    erlang:raise(error, function_clause, [{exe_repl,history,1}]);
local_func(results, [{integer,_,N}], Bs, _Shell, _RT, _Lf, _Ef) ->
    {value,results(N),Bs};
local_func(results, [_Other], _Bs, _Shell, _RT, _Lf, _Ef) ->
    erlang:raise(error, function_clause, [{exe_repl,results,1}]);
...
local_func(catch_exception, [_Other], _Bs, _Shell, _RT, _Lf, _Ef) ->
    erlang:raise(error, function_clause, [{exe_repl,catch_exception,1}]);
...
shell_undef(F,As,Bs) ->
    {{M,F},Desc} = gen_server:call(?MODULE,{get_command_extension,F,true}),
    mad:info(mad,"command extension {~p:~p} \"~s\"",[M,F,Desc]),
    {eval,erlang:make_fun(M,F,length(As)),As,Bs}.

help() ->
    shell_default:help(),
    io:format("** exe command extensions **~n",[]),
    H = fun({{_M,_F}, Desc}) ->
        io:format("~s~n",[Desc])
    end,
    lists:foreach(H,gen_server:call(?MODULE,{get_extensions,command})),
    ok.
...
pp(V, I, RT) ->
    exe_pretty:print(V, I, columns(), ?LINEMAX, ?CHAR_MAX,
                        record_print_fun(RT)).
...
history(L) when is_integer(L), L >= 0 -> set_env(stdlib, shell_history_length,     L, ?DEF_HISTORY).
results(L) when is_integer(L), L >= 0 -> set_env(stdlib, shell_saved_results,      L, ?DEF_RESULTS).
catch_exception(Bool)                 -> set_env(stdlib, shell_catch_exception, Bool, ?DEF_CATCH_EXCEPTION).
prompt_func(String)                   -> set_env(stdlib, shell_prompt_func,   String, ?DEF_PROMPT_FUNC).
