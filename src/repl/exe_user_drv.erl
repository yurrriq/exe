...
start()                    -> spawn(exe_user_drv, server, ['tty_sl -c -e',{shell,start,[init]}]).
start([Pname])             -> spawn(exe_user_drv, server, [Pname,{shell,start,[init]}]);
start([Pname|Args])        -> spawn(exe_user_drv, server, [Pname|Args]);
start(Pname)               -> spawn(exe_user_drv, server, [Pname,{shell,start,[init]}]).
start(Pname, Shell)        -> spawn(exe_user_drv, server, [Pname,Shell]).
start(Iname, Oname, Shell) -> spawn(exe_user_drv, server, [Iname,Oname,Shell]).

interfaces(UserDrv) ->
    case process_info(UserDrv, dictionary) of
	     {dictionary,Dict} -> case lists:keysearch(current_group, 1, Dict) of
	                               {value,Gr={_,Group}} when is_pid(Group) -> [Gr];
		                                                                 _ -> [] end;
	                     _ -> [] end.

server(Pid, Shell) when is_pid(Pid) -> server1(Pid, Pid, Shell);
server(Pname, Shell)                -> process_flag(trap_exit, true),
                                       case catch open_port({spawn,Pname}, [eof]) of
	                                        {'EXIT', _} -> user:start();
	                                               Port -> server1(Port, Port, Shell) end.
...
server1(Iport, Oport, Shell) ->
	...
	case init:get_argument(remsh) of
		...
		RGr = exe_group:start(self(), RShell, rem_sh_opts(ANode)),
		{RGr,RShell};
	    E when E =:= error ; E =:= {ok,[[]]} ->
		{exe_group:start(self(), Shell),Shell}
	end,
...
switch_cmd({ok,[{atom,_,s},{atom,_,Shell}],_}, Iport, Oport, Gr0) ->
    Pid = exe_group:start(self(), {Shell,start,[]}),
...
switch_cmd({ok,[{atom,_,r},{atom,_,Node},{atom,_,Shell}],_},
	   Iport, Oport, Gr0) ->
    Pid = exe_group:start(self(), {Node,Shell,start,[]}),
    Gr = gr_add_cur(Gr0, Pid, {Node,Shell,start,[]}),
    switch_loop(Iport, Oport, Gr);
